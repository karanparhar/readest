...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  await validateUserAndToken(req.headers['authorization']);

  // Translation is unmetered — every user routes through the DeepL Pro
  // endpoint (the plan-gated Free/Pro split was removed with billing).
  ...
  try {
    const translations = await Promise.all(
      text.map(async (singleText) => {
        if (!singleText?.trim()) {
          return { text: '', detected_source_language: sourceLang };
        }
