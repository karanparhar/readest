...
describe('transferStore', () => {
  ...
  describe('transfer kind', () => {
    ...
    test('getQueueStats counts transfers', () => {
      useTransferStore.getState().addTransfer('h1', 'B1', 'upload');
      expect(useTransferStore.getState().getQueueStats().total).toBe(1);
      expect(useTransferStore.getState().getQueueStats().pending).toBe(1);
    });
  ...
  describe('selectActiveBookDownloadProgress', () => {
    ...
    test('ignores uploads and deletes', () => {
      expect(
        select(
