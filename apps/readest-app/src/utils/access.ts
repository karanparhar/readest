...
/**
 * Master switch for the offline-audio premium paywall. ON: pre-downloading
 * TTS audio requires a {@link TTS_CACHE_PLANS} plan. Flipping it off ungates
 * every plan. The automatic playback cache (audio kept as the user listens)
 * is unaffected — only the explicit download UI is gated.
 */
